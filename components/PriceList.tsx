...
export default function PriceList({ data, internalPage = 0, onInternalPageChange }: { 
  data: SlideData, 
  internalPage?: number,
  ...
  return (
    <div className="flex justify-center items-center w-full max-w-[1200px] mx-auto px-4 md:px-16 py-8 md:py-16 h-full box-border max-md:pt-24 relative">
      <motion.div 
        ...
        className="w-full bg-white/5 backdrop-blur-sm border border-brand-brown/20 rounded-[40px] p-8 md:p-16 relative flex flex-col h-full md:h-auto min-h-[600px]"
      >
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <AnimatePresence mode="wait">
            {internalPage === 0 && (
              <motion.div 
                ...
                className="mb-12 text-center md:text-left relative border border-brand-brown/20 p-4"
              >
                {image && (
          ...
          {internalPage > 0 && (
            <div className="mb-8 flex justify-between items-center border-b border-brand-brown/20 pb-4">
               <h3 className="text-[1.5rem] md:text-[2rem] font-bold text-brand-brown opacity-80 uppercase tracking-widest">
                ...
                {currentPageItems.map((item, index) => {
                  ...
                  return (
                    <motion.div 
                      ...
                      className="flex flex-col border-b border-brand-brown/10 pb-3 group hover:border-brand-brown/30 transition-colors relative"
                    >
                      <span className="absolute -top-2 left-0 text-[6px] opacity-40">pub:9/10/11</span>
                      <div className="flex justify-between items-baseline mb-1">
                        <span className="text-[1.2rem] md:text-[1.5rem] font-bold text-brand-brown/90 group-hover:text-brand-brown transition-colors">
                          {item.name}
                        </span>
                        <span className="text-[1.2rem] md:text-[1.4rem] font-bold text-brand-brown font-mono">
                          {item.price}
                        </span>
                      </div>
                      <div className="flex justify-between items-end">
                        {item.description && (
                          <span className="text-[0.9rem] opacity-60 italic">
                            {item.description}
                          </span>
                        )}
                        {product && product.slug && (
                          <Link 
                            ...
                            className="text-[0.7rem] font-bold text-brand-brown border border-brand-brown/30 px-3 py-1 rounded-full hover:bg-brand-brown hover:text-white transition-all ml-4 mb-1"
                          >
                            Részletek
                          </Link>
                        )}
